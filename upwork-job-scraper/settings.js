...
async function initializeSettings() {
  ...
  try {
    ...
    // Proposal polling toggle
    const proposalPollingToggle = document.getElementById("proposal-polling-toggle");
    chrome.storage.sync.get("proposalPollingEnabled", (data) => {
      proposalPollingToggle.checked = data.proposalPollingEnabled !== false;
    });
    proposalPollingToggle.addEventListener("change", (event) => {
      const isEnabled = event.target.checked;
      const frequency = parseInt(
        document.getElementById("proposal-polling-frequency").value || 10
      );
      chrome.runtime.sendMessage({
        type: "updateProposalPolling",
        enabled: isEnabled,
        frequency: frequency,
      });
      addToActivityLog(
        `Proposal polling ${isEnabled ? "enabled" : "disabled"} (frequency: ${frequency} min)`
      );
    });

    // Proposal polling frequency
    const proposalPollingFrequency = document.getElementById("proposal-polling-frequency");
    chrome.storage.sync.get("proposalPollingFrequency", (data) => {
      proposalPollingFrequency.value = data.proposalPollingFrequency || 10;
    });
    proposalPollingFrequency.addEventListener("change", (event) => {
      const frequency = parseInt(event.target.value || 10);
      const isEnabled = proposalPollingToggle.checked;
      chrome.runtime.sendMessage({
        type: "updateProposalPolling",
        enabled: isEnabled,
        frequency: frequency,
      });
      addToActivityLog(`Proposal polling frequency updated to ${frequency} minutes`);
    });

    // Check proposals button
    document
      .getElementById("check-proposals")
      .addEventListener("click", checkProposals);

    // Clear proposals button
    document
      .getElementById("clear-proposals")
      .addEventListener("click", clearAllProposals);

    // Auto-submission toggle
    const autoSubmissionToggle = document.getElementById("auto-submission-toggle");
    chrome.storage.local.get("autoSubmissionEnabled", (data) => {
      autoSubmissionToggle.checked = data.autoSubmissionEnabled === true;
    });
    autoSubmissionToggle.addEventListener("change", (event) => {
      const isEnabled = event.target.checked;
      const delay = parseInt(
        document.getElementById("auto-submission-delay").value || 5
      ) * 1000; // Convert to milliseconds
      chrome.storage.local.set(
        {
          autoSubmissionEnabled: isEnabled,
          autoSubmissionDelay: delay,
        },
        () => {
          chrome.runtime.sendMessage({
            type: "updateAutoSubmission",
            enabled: isEnabled,
            delay: delay,
          });
          addToActivityLog(
            `Auto-submission ${isEnabled ? "enabled" : "disabled"}`
          );
        }
      );
    });

    // Auto-submission delay
    const autoSubmissionDelay = document.getElementById("auto-submission-delay");
    chrome.storage.local.get("autoSubmissionDelay", (data) => {
      const delaySeconds = (data.autoSubmissionDelay || 5000) / 1000;
      autoSubmissionDelay.value = delaySeconds;
    });
    autoSubmissionDelay.addEventListener("change", (event) => {
      const delay = parseInt(event.target.value || 5) * 1000;
      const isEnabled = autoSubmissionToggle.checked;
      chrome.storage.local.set(
        {
          autoSubmissionEnabled: isEnabled,
          autoSubmissionDelay: delay,
        ...
        () => {
          ...
          addToActivityLog(`Auto-submission delay updated to ${delay / 1000} seconds`);
        }
      );
    });

    // Start auto-submission button
    document
      .getElementById("start-auto-submission")
      .addEventListener("click", startAutoSubmission);
...
async function loadProposals() {
  try {
    ...
    if (response && response.success && response.proposals) {
      addProposalEntries(response.proposals);
    }
  } catch (error) {
...
function addProposalEntries(proposals) {
  const container = document.getElementById("proposals-container");
  container.innerHTML = "";

  if (proposals.length === 0) {
    const emptyMessage = document.createElement("div");
    emptyMessage.textContent = "No proposals stored yet.";
    emptyMessage.style.padding = "1em";
    emptyMessage.style.color = "#8d8c8c";
    container.appendChild(emptyMessage);
    return;
  }

  // Sort proposals by stored/updated time, newest first
  proposals.sort(
    (a, b) =>
      (b.storedAt || b.updatedAt || 0) - (a.storedAt || a.updatedAt || 0)
  );

  for (const proposal of proposals) {
    const proposalElement = document.createElement("div");
    proposalElement.className = "job-item";
    ...
    const title = document.createElement("div");
    title.className = "job-title";
    title.textContent = proposal.jobTitle || proposal.jobUrl || "Unknown Job";
    title.addEventListener("click", () => toggleProposalDetails(proposalElement));
    ...
    statusBadge.textContent = proposal.status || "pending";
    statusBadge.style.padding = "0.25em 0.5em";
    ...
    timeSpan.style.color = "#8d8c8c";
    timeSpan.style.fontSize = "0.9em";
    const storedTime = proposal.storedAt || proposal.updatedAt || Date.now();
    timeSpan.textContent = getRelativeTime(storedTime);
    ...
    openButton.addEventListener("click", () => {
      if (proposal.jobUrl) {
        chrome.tabs.create({ url: proposal.jobUrl });
      }
    });
    ...
    header.appendChild(title);
    header.appendChild(rightSection);
    proposalElement.appendChild(header);

    // Create details section (hidden by default)
    const details = document.createElement("div");
    details.className = "job-details";
    details.style.display = "none";
    const proposalTextPreview =
      proposal.proposalText?.substring(0, 200) || "No proposal text";
    details.innerHTML = `
      <p><strong>Job URL:</strong> <a href="${proposal.jobUrl}" target="_blank">${proposal.jobUrl}</a></p>
      ${proposal.bidAmount ? `<p><strong>Bid Amount:</strong> ${proposal.bidAmount}</p>` : ""}
      ${proposal.estimatedHours ? `<p><strong>Estimated Hours:</strong> ${proposal.estimatedHours}</p>` : ""}
      <p><strong>Proposal Text Preview:</strong> ${proposalTextPreview}${proposal.proposalText?.length > 200 ? "..." : ""}</p>
      ${proposal.errorMessage ? `<p><strong>Error:</strong> ${proposal.errorMessage}</p>` : ""}
      <p><strong>Stored:</strong> ${proposal.storedAtHuman || "N/A"}</p>
    `;
    proposalElement.appendChild(details);

    container.appendChild(proposalElement);
  }
}

function toggleProposalDetails(proposalElement) {
  const details = proposalElement.querySelector(".job-details");
  if (details) {
    details.style.display = details.style.display === "none" ? "block" : "none";
  }
}
...
async function checkProposals() {
  try {
    showAlert("Checking for new proposals...", "info");
    addToActivityLog("Manual proposal check initiated");
    ...
    if (response && response.success && response.result) {
      const { newCount, totalCount } = response.result;
      if (newCount > 0) {
        showAlert(
          `Found ${newCount} new proposal(s)! Total: ${totalCount}`,
          "success"
        );
        addToActivityLog(`Found ${newCount} new proposal(s)`);
      } else {
        showAlert("No new proposals found.", "info");
        addToActivityLog("No new proposals found");
      }
      // Reload proposals to show updated list
      await loadProposals();
    } else {
      throw new Error(response?.error || "Failed to check proposals");
    }
  } catch (error) {
    console.error("Error checking proposals:", error);
    showAlert(`Failed to check proposals: ${error.message}`, "error");
    addToActivityLog(`Failed to check proposals: ${error.message}`);
  }
}
...
function clearAllProposals() {
  ...
  chrome.storage.local.remove(["storedProposals"], () => {
    document.getElementById("proposals-container").innerHTML = "";
    addToActivityLog("All proposals cleared");
    showAlert("All proposals cleared successfully", "success");
  });
}

async function startAutoSubmission() {
  try {
    showAlert("Starting auto-submission process...", "info");
    addToActivityLog("Auto-submission started manually");

    // Enable auto-submission if not already enabled
    chrome.storage.local.set({ autoSubmissionEnabled: true }, () => {
    ...
    showAlert(`Failed to start auto-submission: ${error.message}`, "error");
    addToActivityLog(`Failed to start auto-submission: ${error.message}`);
  }
}
