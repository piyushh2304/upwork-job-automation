...
try {
  ...
  function updateAlarm() {
    ...
    // Update proposal polling alarm
    if (proposalPollingEnabled) {
      chrome.alarms.clear("checkProposals");
      chrome.alarms.create("checkProposals", {
        delayInMinutes: proposalPollingFrequency,
        periodInMinutes: proposalPollingFrequency,
      });
      console.log(
        `Proposal polling scheduled every ${proposalPollingFrequency} minutes`
      );
    } else {
      chrome.alarms.clear("checkProposals");
      console.log("Proposal polling alarm cleared");
    }
  }
  ...
  async function initializeExtension(source) {
    ...
    try {
      ...
      const data = await chrome.storage.sync.get([
        ...
        "lastViewedTimestamp",
        "schedule",
        "proposalPollingEnabled",
        "proposalPollingFrequency",
      ]);
      ...
      schedule = data.schedule || schedule;
      proposalPollingEnabled =
        data.proposalPollingEnabled !== undefined
          ? data.proposalPollingEnabled
          : true;
      proposalPollingFrequency = data.proposalPollingFrequency || 10;
      ...
      if (jobScrapingEnabled) {
        ...
        // Still update alarm to set proposal polling
        updateAlarm();
      }
  ...
  chrome.alarms.onAlarm.addListener((alarm) => {
    try {
      ...
      } else if (alarm.name === "checkProposals" && proposalPollingEnabled) {
        // Check for proposals asynchronously
        checkForNewProposals().catch((error) => {
          console.error("Error checking for proposals:", error);
          logAndReportError("Error checking for proposals", error);
        });
      }
    } catch (error) {
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    try {
      ...
      if (message.type === "updateCheckFrequency") {
        ...
        return true; // Respond asynchronously
      } else if (message.type === "updateProposalStatus") {
        handled = true;
        handleAsyncOperation(async () => {
        ...
        return true; // Respond asynchronously
      } else if (message.type === "getAllProposals") {
        handled = true;
        handleAsyncOperation(async () => {
        ...
        return true; // Respond asynchronously
      } else if (message.type === "checkProposals") {
        handled = true;
        handleAsyncOperation(async () => {
        ...
        return true; // Respond asynchronously
      } else if (message.type === "updateProposalPolling") {
        proposalPollingEnabled = message.enabled;
        proposalPollingFrequency = message.frequency || 10;
        chrome.storage.sync.set(
          {
            proposalPollingEnabled: proposalPollingEnabled,
            proposalPollingFrequency: proposalPollingFrequency,
          },
          () => {
            updateAlarm();
            addToActivityLog(
              `Proposal polling ${proposalPollingEnabled ? "enabled" : "disabled"} (frequency: ${proposalPollingFrequency} min)`
            );
          }
        );
        handled = true;
        sendResponse({ success: true });
      } else if (message.type === "markJobAsSubmitted") {
        handled = true;
        handleAsyncOperation(async () => {
        ...
        handled = true;
        handleAsyncOperation(async () => {
          await processNextJobInQueue();
          return {};
        });
        return true; // Respond asynchronously
      } else if (message.type === "updateAutoSubmission") {
        handled = true;
        chrome.storage.local.set(
          {
            autoSubmissionEnabled: message.enabled,
            autoSubmissionDelay: message.delay || 5000,
          },
          () => {
            addToActivityLog(
              `Auto-submission ${message.enabled ? "enabled" : "disabled"}`
            );
            if (message.enabled) {
